...
export const generateWhatsAppMessage = (data: WhatsAppData): string => {
    ...
    const inscripcionText = (data.fechaInicioInscripcion && data.fechaFinInscripcion)
        ? `Desde *${formatDate(data.fechaInicioInscripcion)}* hasta el *${formatDate(data.fechaFinInscripcion)}*`
        : 'Consultar en Campus';

    // Format Schedules
    const cronogramaText = data.horarios.length > 0
        ? data.horarios.map(s => `• ${s}`).join('\n')
        : 'A confirmar';
    ...
    if (cleanDir.includes('virtual') || cleanDir.includes('online') || cleanDir.includes('zoom') || cleanDir.includes('meet')) {
        modalidad = 'Online 💻';
