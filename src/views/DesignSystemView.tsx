...
import ConvocatoriaCardPremium, { ConvocatoriaDetailProps } from '../components/ConvocatoriaCardPremium';
...
const DesignSystemView: React.FC = () => {
    ...
    const sampleData: ConvocatoriaDetailProps = {
        ...
        descripcion: 'Esta práctica se desarrolla en la consultora de recursos humanos Randstad. El objetivo es que los estudiantes desarrollen habilidades en el área de Talent Acquisition (Selección de Personal), participando activamente en procesos de reclutamiento y selección de perfiles para diversas industrias.',
        actividades: [
        ...
        logoUrl: 'https://ui-avatars.com/api/?name=Randstad&background=0D8ABC&color=fff&size=128&rounded=false&bold=true',
        status: 'abierta'
    };
    ...
    const sampleDataClinica: ConvocatoriaDetailProps = {
        ...
        orientacion: 'Clinica',
        descripcion: 'Práctica clínica en el Hospital Regional. Rotación por servicios de Salud Mental y Adicciones.',
    ...
    return (
        <div className={`min-h-screen p-8 transition-colors duration-300 ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>

            {/* Header with Dark Mode Toggle */}
            <div className="max-w-7xl mx-auto mb-12 flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Design System: Premium Cards</h1>
                    <p className="text-slate-500 dark:text-slate-400">Prototipo de visualización para nuevas convocatorias.</p>
                </div>
                <button
                    onClick={toggleTheme}
