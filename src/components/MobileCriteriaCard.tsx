...
const MobileCriteriaCard: React.FC<MobileCriteriaCardProps> = ({
  ...
  const Badge = ({
    ...
    >
      ...
      <span className="text-sm font-black font-mono opacity-100">{subtext}</span>
    </div>
  );
        ...
        <div className="flex justify-between items-end mb-6 pb-4 border-b border-slate-100 dark:border-white/5">
          <div>
            ...
            <div className="flex items-baseline gap-1.5">
              <span
                className={`text-6xl font-black tracking-tighter leading-none ${isTotalComplete ? "text-transparent bg-clip-text bg-gradient-to-br from-emerald-500 to-teal-600 dark:from-emerald-400 dark:to-teal-500" : "text-slate-900 dark:text-white"}`}
              >
          ...
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center border-2 shadow-sm ${isTotalComplete && isRotationComplete && isSpecialtyComplete ? "border-emerald-500 text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20" : "border-slate-100 text-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-800"}`}
          >
            <span className="material-icons !text-2xl">
              {isTotalComplete && isRotationComplete && isSpecialtyComplete
                ? "verified"
                : "hourglass_top"}
            </span>
          </div>
