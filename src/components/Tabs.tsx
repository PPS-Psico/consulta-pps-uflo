...
const Tabs: React.FC<TabsProps> = ({ tabs, activeTabId, onTabChange, onTabClose, className = '' }) => {
  return (
    <div className={`flex flex-col w-full bg-white dark:bg-[#0F172A] rounded-[2rem] shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden relative transition-all duration-300 ${className}`}>
                ...
                {tabs.map(tab => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className={`material-icons !text-lg relative z-10 transition-colors duration-200 ${isActive ? 'text-blue-600 dark:text-blue-400' : ''}`}>
                                {tab.icon}
                            </span>
                            <span className="relative z-10">{tab.label}</span>
                            
                            {tab.badge !== undefined && tab.badge > 0 && (
                                <span className={`ml-2 flex h-5 min-w-[20px] px-1.5 items-center justify-center rounded-full text-[10px] shadow-sm relative z-10 transition-colors duration-200 ${isActive ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300' : 'bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                                    {tab.badge}
                                </span>
                            )}
