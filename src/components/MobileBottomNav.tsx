...
interface MobileBottomNavProps {
  tabs: NavTab[];
  activeTabId: TabId; // Kept for backward compatibility if needed, but we'll derive active from path mostly
  onTabChange?: (tabId: TabId) => void; // Optional now
}

const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ tabs }) => {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg border-t border-slate-200/80 dark:border-slate-700/80 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] dark:shadow-[0_-2px_10px_rgba(0,0,0,0.2)] z-40">
      <div className="flex justify-around items-start h-16 pt-2">
        {tabs.map((tab) => {
          const isActive = location.pathname.startsWith(tab.path) || (tab.path === '/student' && location.pathname === '/student');
          return (
            <button
              key={tab.id}
