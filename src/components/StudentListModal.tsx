...
const StudentListModal: React.FC<StudentListModalProps> = ({ isOpen, onClose, title, students, headers, description, onStudentClick }) => {
        ...
        <table className="w-full text-sm text-left">
            ...
            <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-200/70 dark:divide-slate-700/70">
                {filteredStudents.map((student, index) => (
                    <tr key={student.ppsId || `${student.legajo}-${index}`} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors">
                        {headers!.map(header => (
                            <td key={header.key} className="px-4 py-3 text-slate-700 dark:text-slate-300">{student[header.key]}</td>
                        ))}
                        {onStudentClick && (
    ...
    const renderList = () => (
        <ul className="divide-y divide-slate-200/70 dark:divide-slate-700/70">
            {filteredStudents.map((student, index) => {
                ...
                return (
                    <li key={key} className="p-4 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            <div className="flex-1 min-w-0">
                                <p className="font-bold text-slate-800 dark:text-slate-100 truncate" title={student.nombre}>{student.nombre}</p>
    ...
    return (
        ...
        >
            <div onClick={(e) => e.stopPropagation()} className="relative w-[95vw] sm:w-full max-w-3xl max-h-[85dvh] sm:max-h-[90vh] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-slate-200/70 dark:border-slate-800 animate-scale-in">
                <div className="p-6 flex-shrink-0 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            <h2 id="student-list-modal-title" className="text-xl font-black text-slate-800 dark:text-slate-100 tracking-tight">{title}</h2>
                            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1">{filteredStudents.length} de {students.length} resultados</p>
                            {description && <div className="mt-2 text-xs p-2 bg-slate-100 dark:bg-slate-800 rounded-lg text-slate-600 dark:text-slate-300 font-medium">{description}</div>}
                        </div>
                        <button type="button" onClick={onClose} className="p-2 rounded-full text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 dark:hover:text-slate-300 transition-colors" aria-label="Cerrar modal">
                            <span className="material-icons !text-xl">close</span>
                        </button>
                    </div>
                    <div className="relative mt-4">
                        <input type="search" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Buscar..." className="w-full pl-10 pr-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-200 placeholder-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/30 outline-none transition-all shadow-sm"/>
                ...
                <div className="overflow-y-auto flex-grow bg-slate-50/30 dark:bg-slate-900/30 custom-scrollbar">
                    {filteredStudents.length > 0 ? (
                        ...
                        <div className="text-center py-12 px-6">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 font-medium">No se encontraron resultados que coincidan con la búsqueda.</p>
                        </div>
                    )}
                </div>
            </div>
