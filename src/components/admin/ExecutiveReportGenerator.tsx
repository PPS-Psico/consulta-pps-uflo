...
const ExecutiveReportGenerator: React.FC<ExecutiveReportGeneratorProps> = ({
  ...
  const {
    ...
    isLoading,
    error,
  } = useExecutiveReportData({ reportType, enabled: isReportGenerated, isTestingMode });
      ...
      <div className="no-print p-6 bg-slate-50/70 dark:bg-slate-800/50 rounded-xl border border-slate-200/60 dark:border-slate-700 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex-grow">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Elige el tipo de balance que deseas generar.
          </p>
        </div>
        <div className="w-full sm:w-auto flex flex-col sm:flex-row gap-3">
          <button
            ...
            disabled={isLoading}
            className="w-full sm:w-auto bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 font-bold py-2.5 px-5 rounded-lg text-sm transition-colors shadow-md disabled:bg-slate-300 disabled:cursor-not-allowed flex items-center justify-center gap-2 hover:bg-slate-100 dark:hover:bg-slate-600"
          >
            Balance 2024
          </button>
          <button
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full sm:w-auto bg-blue-600 text-white font-bold py-2.5 px-5 rounded-lg text-sm transition-colors shadow-md disabled:bg-slate-400 disabled:cursor-not-allowed flex items-center justify-center gap-2 hover:bg-blue-700"
          >
