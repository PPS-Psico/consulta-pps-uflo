                ...
                <div className="overflow-hidden cursor-default" onClick={(e) => e.stopPropagation()}>
                    <div className="p-4 pl-5 pt-6">
                        ...
                        <div className="mt-4 text-[10px] text-slate-400 text-right pb-4 px-4">
                             Actualizado: {formatDate(req[FIELD_ULTIMA_ACTUALIZACION_PPS] || req.createdTime)} • {req._daysSinceUpdate} días sin cambios
                        </div>
                    </div>
                </div>
            </div>
...
const SolicitudesManager: React.FC<SolicitudesManagerProps> = ({ isTestingMode = false }) => {
    ...
    const { data: requestsData, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            return data.map((req: any) => {
                ...
                const updatedAt = new Date(req[FIELD_ULTIMA_ACTUALIZACION_PPS] || req.created_at);
                const daysSinceUpdate = Math.floor((new Date().getTime() - updatedAt.getTime()) / (1000 * 3600 * 24));
    ...
    const { activeList, historyList } = useMemo(() => {
        ...
        history.sort((a, b) => new Date(b.createdTime).getTime() - new Date(a.createdTime).getTime());
