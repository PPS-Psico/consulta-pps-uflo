...
const ConvocatoriaCardPremium: React.FC<ConvocatoriaDetailProps> = ({
    ...
    const getButtonConfig = () => {
        ...
        const baseClasses = "px-6 py-2.5 rounded-[14px] font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all duration-300 min-w-[140px] md:min-w-[160px] relative overflow-hidden h-10 md:h-11 border shadow-sm";
        ...
        if (estadoLower === 'no_seleccionado' || status?.toLowerCase() === 'cerrada' || status?.toLowerCase() === 'cerrado') {
            ...
            <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-start justify-between gap-6 relative z-10">
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-3">
                    <div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-2">
                            <span className={`px-2.5 py-0.5 rounded-lg text-[10px] uppercase font-black tracking-wider border ${theme.tag}`}>
                                {orientacion}
                            </span>
                            ...
                            {(() => {
                                ...
                                return (
                                    ...
                                    >
                                        <span className={`material-icons !text-sm ${isVirtual ? '' : 'text-indigo-500 group-hover/addr:text-indigo-700'} transition-colors`}>
                                            {isVirtual ? 'wifi' : 'location_on'}
                                        </span>
                                        <span className={`whitespace-normal leading-tight ${!isVirtual && 'group-hover/addr:underline decoration-indigo-500/30 underline-offset-2'} transition-all`}>
                                            {direccion}
                                        </span>
                                    </TagComponent>
                                );
                ...
                <div className="flex items-center gap-3 self-start">
                    ...
                    >
                        ...
                        {btnConfig.icon && <span className="material-icons !text-lg relative z-10">{btnConfig.icon}</span>}
                    </button>
            ...
            <div className={`grid transition-all duration-500 ease-in-out ${isExpanded ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                <div className="overflow-hidden">
                    ...
                    <div className="px-6 md:px-8 pb-6">
                        <div className={`grid grid-cols-2 md:grid-cols-3 ${reqCv ? 'lg:grid-cols-5' : 'lg:grid-cols-4'} gap-3`}>
                            ...
                            {showRequisitoMetric && (
                                <MetricItem icon="verified_user" label="Requisito" value={requisitoObligatorio} theme="amber" />
                            )}

                            {showCvMetric && (
                                <MetricItem icon="description" label="Documentación" value="Obligatorio Adjuntar CV" theme="amber" />
                            )}
                        </div>
                    </div>
                    ...
                    <div className="p-6 md:p-8 flex flex-col lg:flex-row gap-10">
                        ...
                        <div className="lg:w-[45%] flex flex-col gap-8">
                            ...
                            {(() => {
                                const isComplex = horariosCursada.includes(';') || horariosCursada.includes('\n') || horariosCursada.length > 40;
                                if (!isComplex) return null;

                                const scheduleItems = horariosCursada.split(';').map(s => s.trim()).filter(Boolean);

                                return (
                                    <div className="flex flex-col">
                                        <SectionHeader icon="calendar_month" title="Días y Horarios" color="text-blue-400" />
                                        <div className="flex flex-col gap-2 w-full">
                                            {scheduleItems.map((item, i) => (
                                                <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800/30">
                                                    <span className="material-icons !text-sm text-blue-500 mt-0.5">check_circle</span>
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-3 w-full">
                                    {actividades.map((act, i) => (
                                        ...
                                        `}>
                                            <div className={`w-2 h-2 rounded-full mr-3 flex-shrink-0 bg-gradient-to-br ${theme.gradient || 'from-blue-400 to-indigo-400'}`} />
...
const SectionHeader: React.FC<{ icon: string; title: string; color?: string }> = ({ icon, title, color = "text-slate-300" }) => (
...
const MetricItem: React.FC<{
    ...
    const themeStyles = {
        slate: 'bg-slate-50/50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-800 text-slate-400',
        indigo: 'bg-indigo-50/60 dark:bg-indigo-900/10 border-indigo-100 dark:border-indigo-800/30 text-indigo-500',
        ...
        amber: 'bg-amber-50/80 dark:bg-amber-900/10 border-amber-100 dark:border-amber-800/30 ring-4 ring-amber-50/50 dark:ring-transparent text-amber-500'
    };
    ...
    `} title={value}>
            ...
            <div className="text-[10px] uppercase font-black opacity-70 tracking-wider leading-none">{label}</div>
...
const TimelinePoint: React.FC<TimelinePointProps> = ({
    title, date, icon, isCenter, isLast,
    colorClass, bgClass, borderClass, ringClass
}) => {
    return (
        <div className={`relative flex md:flex-col items-center md:items-start gap-4 md:gap-3 group/point ${isCenter ? 'md:items-center' : ''} ${isLast ? 'md:items-end' : ''}`}>
      ...
      `}>
            ...
            <div className={`flex flex-col ${isCenter ? 'md:items-center' : ''} ${isLast ? 'md:items-end' : ''}`}>
