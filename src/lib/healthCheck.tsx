...
export const useHealthMonitor = () => {
  ...
  const runHealthCheck = async () => {
    ...
    try {
      const [databaseOk, authOk, apiOk, cacheOk, storageOk] = await Promise.all([
        checkDatabaseHealth(),
    ...
    try {
      ...
      setMetrics({
        ...
        memoryUsage: memoryData
          ? (memoryData.usedJSHeapSize / memoryData.totalJSHeapSize) * 100
          : 0,
...
export const HealthCheckDisplay: React.FC = () => {
  const { health, metrics, startMonitoring, stopMonitoring, getStatusColor, getStatusIcon } =
    useHealthMonitor();
    ...
    <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          <span className={getStatusColor(health.status)}>{getStatusIcon(health.status)}</span>
