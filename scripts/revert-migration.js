...
if (SUPABASE_URL.includes("PEGAR_AQUI") || SUPABASE_SERVICE_KEY.includes("PEGAR_AQUI")) {
  console.error(
    "❌ ERROR: Pega las credenciales en scripts/revert-migration.js antes de ejecutar."
  );
  process.exit(1);
}
...
async function revertSolicitudes() {
  ...
  console.log(
    `⚠️  Se encontraron ${badRecords.length} registros duplicados/migrados recientemente.`
  );
  ...
  for (let i = 0; i < idsToDelete.length; i += batchSize) {
    ...
    const { error: deleteError } = await supabase.from("solicitudes_pps").delete().in("id", batch);
