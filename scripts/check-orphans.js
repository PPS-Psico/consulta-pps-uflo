...
async function checkOrphans() {
  ...
  for (const p of practicas) {
    ...
    if (isOrphan) {
      ...
      console.log(
        `   Problema: ${!p.estudiante_id ? "Falta Estudiante" : ""} ${!p.lanzamiento_id ? "Falta Lanzamiento" : ""}`
      );
    }
  }
  ...
  if (orphans > 0) {
    console.log("\n💡 RECOMENDACIÓN:");
    console.log(
      "Si estas prácticas son residuos de pruebas o errores de migración, puedes borrarlas."
    );
    console.log(
      "Si son datos históricos valiosos, deberás vincularlas manualmente a un estudiante o lanzamiento desde el Editor DB."
    );
  } else {
