...
const mapSolicitud = (f) => {
  ...
  return {
    ...
    tutor_disponible: cleanText(
      f["¿La institución cuenta con un psicólogo/a que pueda actuar como tutor/a de la práctica?"]
    ),
    ...
    descripcion_institucion: cleanText(
...
async function main() {
  ...
  if (existingStudents)
    existingStudents.forEach((s) => {
      if (s.airtable_id) idMap.set(s.airtable_id, s.id);
    });
  ...
  const { data: existingLaunches } = await supabase
    ...
    .select("id, airtable_id");
  if (existingLaunches)
    existingLaunches.forEach((l) => {
      if (l.airtable_id) idMap.set(l.airtable_id, l.id);
    });
