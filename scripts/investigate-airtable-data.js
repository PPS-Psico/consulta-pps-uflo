...
async function investigate() {
  ...
  if (brokenStudents && brokenStudents.length > 0) {
    for (const s of brokenStudents) {
      ...
      if (!atRecord) {
        console.log(
          `🗑️  ID ${s.airtable_id}: NO EXISTE en Airtable (Borrado en origen). -> SEGURO BORRAR EN DB.`
        );
      } else {
        ...
        if (!nombre && !legajo) {
          console.log(
            `🔴 ID ${s.airtable_id}: Existe en Airtable pero ESTÁ VACÍO (Campos en blanco). -> SEGURO BORRAR.`
          );
        } else {
          console.log(
            `🟡 ID ${s.airtable_id}: En Airtable TIENE DATOS (${nombre || "Sin Nom"} - ${legajo || "Sin Leg"}). -> FALLÓ MIGRACIÓN.`
          );
        }
      }
    }
  ...
  if (brokenConvs && brokenConvs.length > 0) {
    for (const c of brokenConvs) {
      ...
      if (!estudianteLink || estudianteLink.length === 0) {
        console.log(
          `🔴 ID ${c.airtable_id}: En Airtable NO TIENE ESTUDIANTE asignado. -> REGISTRO INCOMPLETO (Basura).`
        );
      } else {
        ...
        const { data: studentInDb } = await supabase
          .from("estudiantes")
          .select("id")
          .eq("airtable_id", studentAirtableId)
          .maybeSingle();

        if (studentInDb) {
          console.log(
            `🟡 ID ${c.airtable_id}: Tiene link a estudiante ${studentAirtableId} y el estudiante EXISTE en DB. -> SE PUEDE REPARAR (Re-vincular).`
          );
        } else {
          console.log(
            `🟠 ID ${c.airtable_id}: Tiene link a estudiante ${studentAirtableId} pero el estudiante NO EXISTE en DB. -> FALTA MIGRAR ESTUDIANTE.`
          );
        }
      }
    }
