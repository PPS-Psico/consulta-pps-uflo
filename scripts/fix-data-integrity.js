...
async function fixSolicitudesNames() {
  ...
  if (solError) {
    console.error("Error leyendo solicitudes:", solError);
    return;
  }
  ...
  if (estError) {
    console.error("Error leyendo estudiantes:", estError);
    return;
  }
...
async function fixFinalizacionLinks() {
  ...
  if (error) {
    console.error("Error leyendo finalizaciones:", error);
    return;
  }
  ...
  if (updates.length > 0) {
    ...
    console.log(
      "   ⚠️ No se pudieron recuperar vínculos (quizás los estudiantes no existen en Airtable o no se migraron)."
    );
  }
}
