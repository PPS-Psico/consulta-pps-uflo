...
async function repairPracticeStudentLinks() {
  ...
  let offset = null;
  console.log(
    "   📥 Descargando Prácticas originales de Airtable (campo 'Estudiante Inscripto')..."
  );
  ...
  const { data: sbPracticas } = await supabase
    .from("practicas")
    .select("id, airtable_id, estudiante_id");
  const sbPracticasMap = new Map(); // AirtableID -> { sbID, currentStudentId }
  if (sbPracticas)
    sbPracticas.forEach((p) =>
      sbPracticasMap.set(p.airtable_id, { id: p.id, currentStudentId: p.estudiante_id })
    );
  ...
  console.log(
    `   🚀 Reparando ${updates.length} prácticas con vínculos incorrectos o faltantes...`
  );
  ...
  for (let i = 0; i < updates.length; i += BATCH_SIZE) {
    ...
    const promises = batch.map((u) =>
      supabase.from("practicas").update({ estudiante_id: u.estudiante_id }).eq("id", u.id)
    );
