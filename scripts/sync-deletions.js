...
async function syncDeletions() {
  ...
  if (validAirtableIds.size === 0) {
    ...
    console.error(
      "   ⛔ Se aborta el proceso para evitar borrar toda la base de datos accidentalmente."
    );
    return;
  }
  ...
  console.log(
    `   🗑️  Se encontraron ${toDelete.length} registros 'zombies' en Supabase (borrados en Airtable). Eliminando...`
  );
