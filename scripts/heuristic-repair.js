...
async function heuristicRepair() {
  ...
  console.log(
    `   📦 Referencias encontradas en Airtable: ${atStudentIds.size} Estudiantes, ${atLaunchIds.size} Lanzamientos.`
  );
  ...
  const { data: sbLaunches } = await supabase
  ...
  if (updates.length === 0) {
    console.log(
      "   🤷 No se pudo conectar nada nuevo. Es posible que falten los datos maestros (Estudiantes/Lanzamientos) en Supabase."
    );
    return;
  }
