...
async function repairConvocatorias() {
  ...
  if (launches)
    launches.forEach((l) => {
      if (l.airtable_id) launchMap.set(l.airtable_id, l.id);
    });
  ...
  if (updates.length === 0) {
    console.log(
      "   🤷 No se pudo reparar ningún registro (probablemente los estudiantes o lanzamientos tampoco están en Supabase)."
    );
    return;
  }
  ...
  for (let i = 0; i < updates.length; i += BATCH_SIZE) {
    ...
    const promises = batch.map((u) => supabase.from("convocatorias").update(u).eq("id", u.id));
