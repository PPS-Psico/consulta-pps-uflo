...
const SUPABASE_SERVICE_KEY =
  process.env.SUPABASE_SERVICE_ROLE_KEY || "PEGAR_AQUI_TU_SUPABASE_SERVICE_ROLE_KEY";
...
async function loadReferenceData() {
  ...
  students?.forEach((s) => {
    if (s.airtable_id) studentMap.set(s.airtable_id, s.id);
  });

  const { data: launches } = await supabase
    .from("lanzamientos_pps")
    .select("id, airtable_id, nombre_pps, fecha_inicio");
...
async function main() {
  ...
  try {
    const { studentMap, launchMapById, launchMapByNameDate, launchMapByName } =
    ...
    for (let i = 0; i < allAirtableRecords.length; i++) {
      ...
      if (i % 50 === 0)
        process.stdout.write(
          `   Procesando: ${Math.round((i / allAirtableRecords.length) * 100)}%\r`
        );
      ...
      if (sbStudentId && sbLaunchId) {
        ...
        const { data: existingByLogic } = await supabaseOperationWithRetry(() =>
          supabase
            ...
            .eq("estudiante_id", sbStudentId)
            .eq("lanzamiento_id", sbLaunchId)
            .maybeSingle()
        );
          ...
          if (existingByLogic.airtable_id !== record.id) {
            ...
            if (existingById && existingById.id !== existingByLogic.id) {
              ...
              await supabaseOperationWithRetry(() =>
                supabase.from("convocatorias").delete().eq("id", existingById.id)
              );
    ...
    console.log(
      `   - Colisiones Eliminadas: ${stats.collisionsFixed} (Corrección 'Duplicate Key')`
    );
