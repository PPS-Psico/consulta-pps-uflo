...
if (SUPABASE_URL.includes("PEGAR_TU") || SUPABASE_SERVICE_KEY.includes("PEGAR_TU")) {
  console.error(
    "❌ ERROR: Edita el archivo scripts/delete-orphans.js y pega las credenciales (SERVICE KEY)."
  );
  process.exit(1);
}
...
async function deleteOrphans() {
  ...
  if (stError) {
    console.error("Error leyendo estudiantes:", stError);
    return;
  }
  ...
  if (practicasToDelete.length > 0) {
    console.log(
      `   ⚠️  Se encontraron ${practicasToDelete.length} prácticas huérfanas (Desconocido). Eliminando...`
    );
    const { error: delError } = await supabase
      .from("practicas")
      .delete()
      .in("id", practicasToDelete);
  ...
  if (solicitudesToDelete.length > 0) {
    console.log(
      `   ⚠️  Se encontraron ${solicitudesToDelete.length} solicitudes huérfanas. Eliminando...`
    );
    const { error: delError } = await supabase
      .from("solicitudes_pps")
      .delete()
      .in("id", solicitudesToDelete);
