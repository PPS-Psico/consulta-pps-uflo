...
async function advancedRepair() {
  ...
  console.log(
    `   📦 Faltantes detectados: ${missingStudentIds.size} Estudiantes, ${missingLaunchIds.size} Lanzamientos.`
  );
  ...
  if (missingStudentIds.size > 0) {
    ...
    if (toInsert.length > 0) {
      const { data: inserted, error: insErr } = await supabase
        .from("estudiantes")
        .upsert(toInsert, { onConflict: "airtable_id" })
  ...
  console.log(
    `   (Si el número es menor a los fantasmas encontrados, es porque en Airtable esos registros tampoco tienen el vínculo)`
  );
}
